...
function getUsersFromDatabase(array) {
  ...
  //  const nArr = Array.from{0: 'id', firstName: FIRSTNAME, year: age};
...
function filterByCondition(arr, condition) {
  ...
  const number2 = arr.filter(condition);
...
function combineAndReverseArrays(arr1, arr2) {
  ...
  const arr = Array.isArray(arr1, arr2);
...
function customFill(arr, value, start = 0, end = arr.length) {
  ...
  if (0 > start > arr.length && 0 > end > arr.length) {
    return false;
  }
...
function customAt(arr, index) {
  ...
  const elIndex = arr.indexOf(index);
...
function customIncludes(arr, element) {
  ...
  const arrIncludes = arr.includes(element);
  return arrIncludes;
